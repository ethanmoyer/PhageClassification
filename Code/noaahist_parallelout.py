...
def run_req(args):
    ...
    if req.meta:
        req.set_metastr()
    multi_queue.put((req.response_list, req.meta_str))
...
class WeatherDataRequest(object):
    def __init__(self, start_date, end_date, lat, lon, flds, stns, meta, name=None):
        ...
        print(len(sorted([date for date in self.dates])))
...
class AllWeatherResponses(object):
    def __init__(self, dest):
        ...
        dest.write(','.join(self.fld_names) + "\n")
        dest.flush()
    ...
    def write_response(self, objs_dict, dest):
        dest.write(self.format_line(objs_dict))
        dest.flush()
...
def req_from_infile_line(line, stns, meta):
    try:
        #print(map(lambda x: x.strip(), line.strip().split("|")))
...
def main(args, update_stations=False):
    ...
    if nprocs:
        ...
        result = pool.map_async(run_req,[(x,multi_queue) for x in reqs])
        #resps = result.get()
        all_resp = AllWeatherResponses(args.outfile)

        while not multi_queue.empty() or not result.ready():
            if not multi_queue.empty():
                (lst,meta) = multi_queue.get()
                
                #Write the result
                [all_resp.write_response(x,args.outfile) for x in lst]        
    else:
